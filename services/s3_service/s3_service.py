...
class S3Service:
    def __init__(self, aws_service):
        self.__s3_client = aws_service.s3_client
    ...
    def get_object(self, bucket_name, object_name):
        try:
            response = self.__s3_client.get_object(Bucket=bucket_name, Key=object_name)
    ...
    def list_objects_v2(self, bucket_name, prefix=None):
        try:
            if prefix:
                ...
                response = self.__s3_client.list_objects_v2(Bucket=bucket_name)
        except Exception as ex:
            return False, str(ex)
    ...
    def upload_file_with_put(self, bucket_name, file_content, object_name):
        ...
        """
        try:
            self.__s3_client.put_object(Bucket=bucket_name, Key=object_name, Body=file_content)
